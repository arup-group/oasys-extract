...
bool CExtractData::Write()
{
	...
	//// now get file and save data
	//CString cPath;
	//CFileDialog cFileDlg(false,							// "Save As" dialog box
	//	L"txt",						// default extension
	//	cPath,							// default file name
	//	OFN_OVERWRITEPROMPT |			// flags
	//	OFN_HIDEREADONLY |
	//	OFN_PATHMUSTEXIST,
	//	L"Web Page (*.html)|*.html||",	// file types
	//	nullptr);						// parent window

	//cFileDlg.m_ofn.lpstrTitle = const_cast<wchar_t*>(L"Export");

	//if (cFileDlg.DoModal() == IDOK)
	//{
	//	cPath = cFileDlg.GetPathName();
	...
	//		&cFileEx))
	//	{
	//		AfxMessageBox(L"Error opening " + cPath);
	//		return;
	//	}

	//	// write file
	//	{
	//		cFile.WriteString(L"<!DOCTYPE html>\n"
	//			L"<html>\n"
	//			L"<head>\n"
	//			L"<meta charset=\"UTF-8\">\n"
	//			L"<meta name=\'viewport\' content=\'width=device-width, initial-scale=1\'>\n"
	//			L"<link rel=\'stylesheet\' href=\'css/reset.css\' />\n"
	//			L"<link rel=\'stylesheet\' href=\'css/oasys.css\' />\n"
	//			L"<style>"
	//			L"    body {\n"
	//			L"        font-size:10pt;\n"
	...
	//			L"    }\n"
	//			L"</style>\n");
	//		cFile.WriteString(L"<title>\n");
	//		cFile.WriteString(m_cTitle);
	//		cFile.WriteString(L"</title>\n"
	//			L"</head>\n"
	//			L"<body>\n");
	//		// Title
	//		cFile.WriteString(L"<h2>");
	//		cFile.WriteString(m_cTitle);
	//		cFile.WriteString(L"</h2>\n");
	//		cFile.WriteString(L"<hr style=\"color:black;background-color:black;height:3px;border:0;\" />\n");

	//		// Introduction
	//		if (!m_cIntro.IsEmpty() && m_iBrief == 0)
	//		{
	//			cFile.WriteString(L"<h3>");
	//			cFile.WriteString(L"Introduction");
	//			cFile.WriteString(L"</h3>\n");

	//			if (m_cIntro.Find(L"<p>") >= 0 ||
	//				m_cIntro.Find(L"<h1>") >= 0 ||
	//				m_cIntro.Find(L"<h2>") >= 0 ||
	//				m_cIntro.Find(L"<h3>") >= 0 ||
	//				m_cIntro.Find(L"<h4>") >= 0 ||
	//				m_cIntro.Find(L"<h5>") >= 0 ||
	//				m_cIntro.Find(L"<h6>") >= 0 ||
	//				m_cIntro.Find(L"<table>") >= 0)
	//			{
	//				// assume formatted html
	//				cFile.WriteString(m_cIntro);
	//			}
	//			else
	//			{
	//				// break into paragraphs
	//				CString cHtml = L"<p>";
	//				const int n = m_cIntro.GetLength();
	//				for (int i = 0; i < n; ++i)
	//				{
	//					wchar_t c = m_cIntro[i];
	//					if (c == 13)			cHtml += L"</p>\n<p>";
	//					else if (c != 10)	cHtml += c;
	//				}
	//				cHtml += L"</p>";
	//				cFile.WriteString(cHtml);
	//			}
	//		}
	...
	//		if (!IsBrief())
	//		{
	//			cFile.WriteString(L"<p class=\"index\" /*style=\"font-size:90%\"*/><a href=\"#index\">Index</a></p>");
	//			cFile.WriteString(L"<hr style=\"color:black;background-color:black;height:1px;border:0;\" />\n");
	//		}

	//		// Body
	//		for (int iItem = 0; iItem < m_cItem.size(); ++iItem)
	//		{
	//			CString cWrite = ProcessHtmlString(m_cItem[iItem]);
	//			cFile.WriteString(cWrite);
	//			if (!IsBrief())
	//			{
	//				cFile.WriteString(L"<p class=\"index\" /*style=\"font-size:90%;*/\"><a href=\"#index\">Index</a></p>");
	//				cFile.WriteString(L"<hr style=\"color:black;background-color:black;height:1px;border:0;\" />\n");
	...
	//		if (!IsBrief())
	//		{
	//			cFile.WriteString(L"<h3><a name=\"index\">Index</a></h3>\n"
	//				L"<p>\n");
	//			for (int iItem = 0; iItem < m_cItem.size(); ++iItem)
	//			{
	//				CString cWrite = IndexHtmlString(m_cItem[iItem]);
	//				cFile.WriteString(cWrite);
	//			}
	//			cFile.WriteString(L"</p>\n");
	//		}
	//		// Footer
	//		cFile.WriteString(L"<hr style=\"color:black;background-color:black;height:1px;border:0;\" />\n");
	//		CTime cDate = CTime::GetCurrentTime();
	//		cFile.WriteString(cDate.Format(L"%d %b %Y at %H:%M<br/>\n"));
	//		CTime cCopy = CTime::GetCurrentTime();
	//		cFile.WriteString(cCopy.Format(L"&copy; Oasys Ltd %Y\n"));
	//		cFile.WriteString(L"</body>\n"
	//			L"</html>\n");
	//	}
	//	cFile.Close();
	//	m_bModified = false;

	//	if ((reinterpret_cast<intptr_t>(ShellExecute(nullptr, L"open", cPath, nullptr, cPath, 0))) <= 32)
	//	{
	//		CString cMsg;
	//		cMsg.Format(L"File %s written", cPath);
	//		AfxMessageBox(cMsg);
	//	}
	//}
	return status;
